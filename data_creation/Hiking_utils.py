...
###############################################################################################
def HikingGenProblemsParams(config):
    """
    ...
    for i in range(config.N):
        ...
        if ProbDescriptorVals[1] < ProbDescriptorVals[0]:
            ProbDescriptorVals[1] = ProbDescriptorVals[0] + 1
...
###############################################################################################
def HikingGenerateProblems(NProbDF, config):
    """
    ...
    for i in range(0, NProbDF.shape[0]):
        ...
        cmd_val_list = NProbDF.iloc[i][config.ProbDescriptors].to_list()
        cmd_val_list = [str(val) for val in cmd_val_list]
        cmd_val_list.insert(0, config.generator_path)
        cmd_val_list.insert(0, "python2")
