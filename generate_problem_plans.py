...
def GenerateProblems(NProbDF, generator_path, planner_path):
    ...
    for i in range(0, NProbDF.shape[0]):
        cmd_line_args = ['python', planner_path, domain_path, NProbDF.at[i, "problem.pddl"],
                         "--search \"astar(lmcut())\""]
        cmd_line = " ".join(cmd_line_args)

        plan_out_path = os.path.join(plans_dir, "plan_" + i.__str__())
        fd = open(plan_out_path,
                  "w")  # todo:the bug is that the plan is not printed to the file.. for some reason the plan file is empty
...
def ExtractPlan(NProbDF):
    ...
    for index, row in NProbDF.iterrows():
        ...
        plan_f = open(curr_plan_path, 'r')
        print("STARTED WITH PLAN: " + str(index))
        print("file size is: " + str(plan_f.__sizeof__()))
        for cnt, line in enumerate(plan_f):
            print(line)
            if re.search("Actual search time", line, re.IGNORECASE) is not None:
                flag = True

            if re.search("Plan length:", line, re.IGNORECASE):
